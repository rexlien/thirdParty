                              ...
                              const uint8_t *data, size_t len, void *userp)
{
  ...
  /* if we receive data for another handle, wake that up */
  if(conn->data != data_s)
    Curl_expire(data_s, 0); /* TODO: fix so that this can be set to 0 for
                               immediately? */
...
#define H2_STREAM_WINDOW_SIZE (1024 * 1024 * 6)
#define H2_SESSION_WINDOW_SIZE (1024 * 1024 * 15)
                          ...
                          const void *mem, size_t len, CURLcode *err)
{
  ...
  rv = nghttp2_session_send(h2);
    ...
    int32_t wininc = H2_SESSION_WINDOW_SIZE - 1 - NGHTTP2_INITIAL_CONNECTION_WINDOW_SIZE;
    
    rv = nghttp2_submit_window_update(conn->proto.httpc.h2,
                                      NGHTTP2_FLAG_NONE, stream->stream_id, wininc);
    if(rv) {
                             ...
                             const char *mem, size_t nread)
{
  ...
  if(conn->data->req.upgr101 == UPGR101_RECEIVED) {
    ...
    int32_t wininc = H2_STREAM_WINDOW_SIZE - 1
                        - NGHTTP2_INITIAL_WINDOW_SIZE;
    rv = nghttp2_submit_window_update(conn->proto.httpc.h2,
                                           NGHTTP2_FLAG_NONE, 0, wininc);
        if(rv) {
             failf(conn->data, "Couldn't update the initial window size");
              return CURLE_HTTP2;
            }
